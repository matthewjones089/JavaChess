...
public class Evaluation {
    ...
    private static int EvaluatePieceScore(Square square, byte position, boolean endGamePhase,
                                            byte knightCount, byte bishopCount, boolean insufficientMaterial)
    {
        int score = 0;

        byte index = position;

        if (square.Piece.PieceColour == chessPiece.ChessPieceColour.Black)
        {
        ...
        if (square.Piece.PieceType == chessPiece.ChessPieceType.Pawn)
        {
            ...
            if (square.Piece.PieceColour == chessPiece.ChessPieceColour.White)
            {
        ...
        else if (square.Piece.PieceType == chessPiece.ChessPieceType.Knight)
        {
            knightCount++;
        ...
        else if (square.Piece.PieceType == chessPiece.ChessPieceType.Bishop)
        {
            bishopCount++;
        ...
        else if (square.Piece.PieceType == chessPiece.ChessPieceType.Rook)
        {
            insufficientMaterial = false;
        }
        else if (square.Piece.PieceType == chessPiece.ChessPieceType.Queen)
        {
            insufficientMaterial = false;
        ...
        else if (square.Piece.PieceType == chessPiece.ChessPieceType.King)
        {
            if (square.Piece.validMoves != null)
            {
    ...
    static void EvaluateBoardScore(Board board)
    {
        ...
        for (byte x = 0; x < 64; x++)
        {
            ...
            if (square.Piece.PieceColour == chessPiece.ChessPieceColour.White)
            {
                board.score += EvaluatePieceScore(square, x, board.endGamePhase,
                    whiteKnightCount, whiteBishopCount, board.InsufficientMaterial);

                if (square.Piece.PieceType == chessPiece.ChessPieceType.King)
                {
                    if (x != 59 && x != 60)
                    {
                        int pawnPos = x - 8;
            ...
            else if (square.Piece.PieceColour == chessPiece.ChessPieceColour.Black)
            {
                board.score -= EvaluatePieceScore(square, x, board.endGamePhase,
                    blackKnightCount, blackBishopCount, board.InsufficientMaterial);
            ...
            if (square.Piece.PieceType == chessPiece.ChessPieceType.Knight)
            {
                knightCount++;

                if (knightCount > 1)
                {
                    board.InsufficientMaterial = false;
                }
            }
        ...
        if (board.InsufficientMaterialActive)
        {
    ...
    private static int CheckPawnWall(Board board, int pawnPos, int kingPos)
    {
        ...
        if (board.Squares[pawnPos].Piece.PieceType != chessPiece.ChessPieceType.None)
        {
            if (board.Squares[pawnPos].Piece.PieceColour == board.Squares[kingPos].Piece.PieceColour)
            {
                if (board.Squares[pawnPos].Piece.PieceType == chessPiece.ChessPieceType.Pawn)
                {
                    return 10;
                }
            }
